...
public class ServiceDao implements ServiceIDao {
    ...
    private static final String GET_BY_SERVICENAME="{CALL get_service_by_name(?)}";
    ...
    public List<Service> getByServiceName(String serviceName1) throws SQLException {
        Service service = null;
        List<Service> serviceList = new ArrayList<>();
        try (Connection connection = baseDao.getConnection();
             CallableStatement callableStatement = connection.prepareCall(GET_BY_SERVICENAME);){
            callableStatement.setString(1,serviceName1);
            ResultSet resultSet = callableStatement.executeQuery();
            while (resultSet.next()){
                Integer serviceId =resultSet.getInt("s.service_id");
                String serviceName = resultSet.getString("s.service_name");
